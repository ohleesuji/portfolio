...
filters.innerHTML=[{key:'all',name:'전체'},...companies].map(c=>`<button type="button" data-company="${c.key}" aria-pressed="${c.key==='kakao'}">${esc(c.name)}</button>`).join('');
function evidenceHTML(c,special){
const originals=c.originals;
const pictures=originals||[{src:c.image,label:c.title}];
return `<figure class="evidence ${special?'full':''} ${originals?'original-evidence':''}"><div class="evidence-gallery ${originals&&originals.length>1?'phone-gallery':''}">${pictures.map(p=>`<div><button type="button" class="evidence-button" data-image="${p.src}" data-title="${esc(p.label)}" aria-label="${esc(p.label)} 자료 확대"><img src="${p.src}" alt="${esc(p.label)}" loading="eager" decoding="async"><span class="zoom">자료 확대 ↗</span></button>${originals?`<p class="image-label">${esc(p.label)}</p>`:''}</div>`).join('')}</div><figcaption>${esc(c.caption||'서비스 화면 및 기획 자료')}</figcaption></figure>`;
}
function caseHTML(c){const special=c.page===22;return `<article class="project ${special?'special':''}" id="${c.id}"><header class="project-header"><p class="eyebrow">${esc(companies.find(x=>x.key===c.company).name)} / PROJECT ${String(c.page-4).padStart(2,'0')}</p><h3>${esc(c.title)}</h3></header><div class="project-body"><div class="project-copy">${c.sections.map((s,i)=>`<section class="${!special&&i===2?'results':''}"><h4>${esc(s.title)}</h4><ul>${s.items.map(t=>`<li>${esc(t)}</li>`).join('')}</ul></section>`).join('')}${c.role?`<dl class="project-meta"><dt>담당 역할</dt><dd>${esc(c.role)}</dd><dt>기획 기여도</dt><dd>${esc(c.contribution)}</dd>${c.team?`<dt>협업 구성</dt><dd>${esc(c.team)}</dd>`:''}</dl>`:''}</div>${evidenceHTML(c,special)}</div></article>`}
document.querySelector('#case-list').innerHTML=companies.map(c=>`<div class="company-group" data-group="${c.key}" ${c.key==='kakao'?'':'hidden'}><header class="company-heading"><div><h3>${esc(c.name)}</h3><p>${esc(c.intro)}</p></div></header>${cases.filter(x=>x.company===c.key).map(caseHTML).join('')}</div>`).join('');
...
window.addEventListener('hashchange',revealHash);selectCompany('kakao');if(location.hash)revealHash();
